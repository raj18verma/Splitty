...
export default function App() {
  ...
  function handleSplitBill(bill, paidByUser, whoIsPaying) {
    ...
    setFriends((friends) =>
      friends.map((friend) => {
        if (friend.id === selectedFriend?.id && whoIsPaying === 'friend') {
          return {
            ...friend,
            balance: friend.balance - (bill - perPersonShare),
          };
        }
        return {
          ...friend,
          balance: whoIsPaying === 'user'
            ? friend.balance - perPersonShare
            : friend.balance + perPersonShare,
        };
      })
    );
    ...
    if (whoIsPaying === 'user') {
      // Add the total amount that others owe to the user to the selected friend's balance
      setFriends((friends) =>
        friends.map((friend) => ({
          ...friend,
...
function FormAddFriend({ onAddFriend }) {
  ...
  return (
    <form className="form-add-friend" onSubmit={handleSubmit}>
      <label>
        <span role="img" aria-label="friends">🧑‍🤝‍🧑</span> Friend name
      </label>
      <input
        type="text"
        value={name}
...
function FormSplitBill({ onSplitBill, friends }) {
  ...
  return (
    <form className="form-split-bill" onSubmit={handleSubmit}>
      ...
      <label>
        <span role="img" aria-label="money">💰</span> Bill value
      </label>
      <input
        type="number"
        value={bill}
      ...
      <label>
        <span role="img" aria-label="money-face">🤑</span> Who is paying the bill
      </label>
      <select
        value={whoIsPaying}
